...
def main(
    ...
    Objective 1 (Solution Pool): Maximizes net profit, then minimizes CVaR with profit
    constraint, using XPRESS solution pool to generate multiple alternatives.

    Objective 2 (Hierarchical Multi-Objective): Combines profit maximization and CVaR
    minimization in a hierarchical multi-objective formulation.

    Args:
        segments: DataFrame with columns: asset, segment_id, exposure, risk_weight,
                  fix_sell_cost, rel_sell_cost, fix_origination_cost, rel_origination_cost,
                  average_profitability_1, average_profitability_2, average_profitability_3
        assets: DataFrame with columns: asset, max_exposure_decrease, max_exposure_increase
        covariance_df: DataFrame with covariance matrix (used for correlation visualization)
    ...
    Returns:
        nextmv.Output containing:
            ...
            - solution_pool: DataFrame with alternative solutions (objective_1 only)
    """
    m = gp.Container()
    ...
    if options.objective == "objective_1":
        ...
        # Stage 2: Minimize CVaR with profit constraint using solution pool
        ...
        raise ValueError(
            f"Invalid objective option: {options.objective}. Must be 'objective_1' or 'objective_2'"
        )
    ...
    if options.export_latex:
        latex_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs", "latex")
        os.makedirs(latex_dir, exist_ok=True)
        min_CVaR.toLatex(
            latex_dir,
            rename={
                "risk_weight_limit": r"\mbox{$\Gamma$}",
                "VaR": r"\mbox{$\zeta$}",
                "average_profitability": r"\mbox{$\mu$}",
                "rel_origination_cost": r"\hat{c}",
                "rel_sell_cost": r"\bar{c}",
                "fix_origination_cost": r"\hat{f}",
                "fix_sell_cost": r"\bar{f}",
                "segment_vars": r"x",
                "segment_increase_vars": r"\hat{x}",
                "segment_decrease_vars": r"\bar{x}",
                "segment_increase_bvars": r"\hat{b}",
                "segment_decrease_bvars": r"\bar{b}",
                "new_total_exposure": r"\hat{E}",
                "portfolio_exposure_vars": r"\hat{e}",
            },
        )
        print(f"\nLaTeX export saved to: {latex_dir}")
